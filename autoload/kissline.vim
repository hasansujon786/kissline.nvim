...
function! kissline#_init() abort
  hi Kissline_cur_mode_active      guibg=#98C379 guifg=#2C323C gui=bold
  hi Kissline_cur_mode_sp_active  guibg=#3E4452 guifg=#98C379
  ...
  let l:highlight_colors={
    ...
    \ 'replace' :'#E06C75',
    \}
  for key in keys(l:highlight_colors)
    exec printf('hi Kissline_mode_%s     guifg=#2C323C guibg=%s gui=bold', key, get(l:highlight_colors,key))
    exec printf('hi Kissline_mode_sp_%s  guibg=#3E4452 guifg=%s', key, get(l:highlight_colors,key))
  endfor
endfunction
